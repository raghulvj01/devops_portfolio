...
export default function BlogPostContent({ post }: { post: BlogPost }) {
  ...
  return (
    <>
      ...
      <section className={`min-h-screen bg-background text-foreground pt-20 pb-48 ${inter.className}`}>
        {/* Header Container */}
        <div className="mx-auto px-4 sm:px-6 lg:px-8 max-w-3xl pt-10">
          <motion.div
            initial="hidden"
            animate="visible"
            variants={fadeIn}
        ...
        >
            <div className="relative w-full h-[40vh] min-h-[300px] md:h-[50vh] min-h-[400px] rounded-3xl overflow-hidden shadow-2xl border border-border/40">
              <Image
                src={post.coverImage}
                alt={post.title}
                fill
        ...
        <div className="mx-auto px-4 sm:px-6 lg:px-8 max-w-3xl">
            <article className={`prose prose-zinc dark:prose-invert prose-lg max-w-none prose-headings:font-bold prose-headings:tracking-tight prose-a:text-primary prose-a:no-underline hover:prose-a:underline prose-img:rounded-2xl prose-img:shadow-lg prose-img:border prose-img:border-border/40 ${inter.className}`}>
                ...
                >
                  ...
                  <div className="text-muted-foreground leading-[1.8] text-lg">
                    <ReactMarkdown 
                      ...
                      components={{
                        ol: ({node, ...props}) => <ol className="bg-secondary/30 p-6 sm:p-8 rounded-2xl border border-border/50 my-8 list-decimal list-outside ml-4 text-foreground/90 space-y-4 marker:text-primary font-medium" {...props} />,
                        ul: ({node, ...props}) => <ul className="bg-secondary/30 p-6 sm:p-8 rounded-2xl border border-border/50 my-8 list-disc list-outside ml-4 text-foreground/90 space-y-4 marker:text-primary font-medium" {...props} />,
                        li: ({node, ...props}) => <li className="pl-2" {...props} />,
                        blockquote: ({node, ...props}) => <blockquote className={`${outfit.className} border-l-4 border-primary/80 bg-gradient-to-r from-primary/5 to-transparent p-6 sm:p-8 rounded-r-2xl my-10 italic text-foreground font-medium text-2xl`} {...props} />,
            ...
            {/* Author Box Footer */}
            <div className="bg-gradient-to-br from-secondary/50 to-background rounded-3xl p-8 border border-border/50 shadow-sm flex flex-col sm:flex-row gap-6 items-center sm:items-start text-center sm:text-left mb-16">
